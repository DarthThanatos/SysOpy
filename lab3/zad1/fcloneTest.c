...
void saveRealTime(){
	...
	fprintf(resultsReal,"mother fclone: %7.2f %d\n",
			motherRealDelta,N);
	fprintf(resultsReal,"children fclone: %7.2f %d\n",
			globalChildrenRealTime,N);
	fprintf(resultsReal,"children + mother fclone: %7.2f %d\n",
...
void saveUserAndSysTime(){
	...
	fprintf(resultsUserAndSys,"mother fclone: %7.2f %d\n",
			motherUserDelta + motherSysDelta ,N);
	fprintf(resultsUserAndSys,"children fclone: %7.2f %d\n",
			globalChildrenUserTime + globalChildrenSysTime,N);
	fprintf(resultsUserAndSys,"children + mother fclone: %7.2f %d\n",
...
void saveUserTime(){
	...
	times(&motherTmsEnd);
	double motherUserDelta = (motherTmsEnd.tms_utime - motherTmsStart.tms_utime)/(double)clockTackts;
	fprintf(resultsUser,"mother fclone: %7.2f %d\n",
			motherUserDelta,N);
	fprintf(resultsUser,"children fclone: %7.2f %d\n",
			globalChildrenUserTime,N);
	fprintf(resultsUser,"children + mother fclone: %7.2f %d\n",
...
void saveSysTime(){
	...
	times(&motherTmsEnd);
	double motherSysDelta = (motherTmsEnd.tms_stime - motherTmsStart.tms_stime)/(double)clockTackts;
	fprintf(resultsSys,"mother fclone: %7.2f %d\n",
			motherSysDelta,N);
	fprintf(resultsSys,"children fclone: %7.2f %d\n",
			globalChildrenSysTime,N);
	fprintf(resultsSys,"children + mother fclone: %7.2f %d\n",
			globalChildrenSysTime + motherSysDelta,N);
	fclose(resultsSys);
}
...
void fcloneApproach(int argc, char *argv[]){
	long clockTackts = sysconf(_SC_CLK_TCK);
	pid_t pid;
	int i;
	if (argc != 2){
	...
	for(i = 0; i<N; i++) {
		...
		if((pid = clone(fn,child_stack,SIGCHLD,NULL))< 0 ){
			printf("fclone error");
			return;
		}
		else if(pid == 0){
