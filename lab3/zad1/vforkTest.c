...
void saveRealTime(){
	...
	fprintf(resultsReal,"mother vfork: %7.2f %d\n",
			motherRealDelta ,N);
	fprintf(resultsReal,"children vfork: %7.2f %d\n",
			globalChildrenRealTime,N);
	fprintf(resultsReal,"children + mother vfork: %7.2f %d\n",
...
void saveUserAndSysTime(){
	...
	fprintf(resultsUserAndSys,"mother vfork: %7.2f %d\n",
			motherUserDelta + motherSysDelta ,N);
	fprintf(resultsUserAndSys,"children vfork: %7.2f %d\n",
			globalChildrenUserTime + globalChildrenSysTime,N);
	fprintf(resultsUserAndSys,"children + mother vfork: %7.2f, %d\n",
...
void saveUserTime(){
	...
	times(&motherTmsEnd);
	double motherUserDelta = (motherTmsEnd.tms_utime - motherTmsStart.tms_utime)/(double)clockTackts;
	fprintf(resultsUser,"mother vfork: %7.2f %d\n",
			motherUserDelta,N);
	fprintf(resultsUser,"children vfork: %7.2f %d\n",
			globalChildrenUserTime,N);
	fprintf(resultsUser,"children + mother vfork: %7.2f %d\n",
...
void saveSysTime(){
	...
	times(&motherTmsEnd);
	double motherSysDelta = (motherTmsEnd.tms_stime - motherTmsStart.tms_stime)/(double)clockTackts;
	fprintf(resultsSys,"mother vfork: %7.2f %d\n",
			motherSysDelta,N);
	fprintf(resultsSys,"children vfork: %7.2f %d\n",
			globalChildrenSysTime,N);
	fprintf(resultsSys,"children + mother vfork: %7.2f, %d\n",
			globalChildrenSysTime + motherSysDelta,N);
	fclose(resultsSys);
}

void vforkApproach(int argc, char *argv[]){
	long clockTackts = sysconf(_SC_CLK_TCK);
	pid_t pid;
	int i;
	if (argc != 2){
